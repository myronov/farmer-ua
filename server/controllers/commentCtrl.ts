...
const commentCtrl = {
  createComment: async (req: IReqAuth, res: Response) => {
    ...
    try {
      const { content, post_id, post_user_id } = req.body;
      ...
      io.to(`${post_id}`).emit('createComment', data);
  ...
  replyComment: async (req: IReqAuth, res: Response) => {
    ...
    try {
      const { content, post_id, post_user_id, comment_root, reply_user } =
        req.body;
      ...
      io.to(`${post_id}`).emit('replyComment', data);
  ...
  updateComment: async (req: IReqAuth, res: Response) => {
    ...
    try {
      ...
      io.to(`${data.post_id}`).emit('updateComment', data);
  ...
  deleteComment: async (req: IReqAuth, res: Response) => {
    ...
    try {
      const comment = await Comments.findOneAndDelete({
        _id: req.params.id,
        $or: [{ user: req.user._id }, { post_user_id: req.user._id }],
      });
      ...
      io.to(`${comment.post_id}`).emit('deleteComment', comment);
